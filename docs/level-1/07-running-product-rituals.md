...
using the table above: name each ritual, its cadence, its one-sentence
purpose, and its attendee list. Then run the "five questions" against just
one of them (your choice) and write two sentences on what you'd change if
attendance data showed one PM had missed it three times in a row without
comment.
